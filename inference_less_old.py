...
def main():
    ...
    for i, (img_batch, mel_batch, frames, coords) in enumerate(
        ...
        for p, f, c in zip(pred, frames, coords):
            y1, y2, x1, x2 = c
            # redimensionne la prédiction
            p = cv2.resize(p.astype(np.uint8), (x2 - x1, y2 - y1))

            # 1) Crée un masque circulaire (opaque à l’intérieur, transparent à l’extérieur)
            h, w = p.shape[:2]
            mask = np.zeros((h, w), dtype=np.uint8)
            center = (w // 2, h // 2)
            radius = int((y2 - y1) * 0.25)  # ajustez ce facteur si besoin
            cv2.circle(mask, center, radius, 255, -1)

            # 2) (Optionnel) adoucir légèrement le contour
            mask = cv2.GaussianBlur(mask, (21, 21), 0)

            # 3) Passe en float [0..1] et empile en 3 canaux
            mask_f = mask.astype(np.float32) / 255.0
            mask_3c = np.stack([mask_f] * 3, axis=-1)

            # 4) Mélange prédiction et frame d’origine selon le masque
            orig_patch = f[y1:y2, x1:x2].astype(np.float32) / 255.0
            pred_f = p.astype(np.float32) / 255.0
