...
def main():
    ...
    wav = audio_utils.load_wav(args.audio, 16000)
    mel = audio_utils.melspectrogram(wav)
    print(mel.shape)
    if np.isnan(mel.reshape(-1)).sum() > 0:
        raise ValueError(
            "Mel contains nan! Using a TTS voice? Add une petite epsilon noise to the wav et réessaie"
        )

    # Préparer les chunks de mel
    mel_chunks = []
    ...
    # === Initialisation pour stocker les positions du cercle ===
    circle_positions = []
    frame_idx = 0

    for batch_i, (img_batch, mel_batch, frames, coords) in enumerate(
        ...
        # … juste après l'initialisation circle_positions, frame_idx, out, etc. …

        for p, f, c in zip(pred, frames, coords):
            # 1) Calcul des coords globales
            y1, y2, x1, x2 = map(int, c)
            h, w = (y2 - y1), (x2 - x1)
            center_local = (w // 2, int(h * 0.75))
            radius_local = int((y2 - y1) * 0.23)
            center_global = (x1 + center_local[0], y1 + center_local[1])

            # 2) Stocker la position
            circle_positions.append(
                {
                    "frame": frame_idx,
                    "center": {"x": center_global[0], "y": center_global[1]},
                    "radius": radius_local,
                }
            )
            ...
            # 4) Ton blending circulaire habituel
            p_resized = cv2.resize(p, (w, h))
            mask = np.zeros((h, w), dtype=np.uint8)
            cv2.circle(mask, center_local, radius_local, 255, -1)
            mask = cv2.GaussianBlur(mask, (21, 21), 0)
            mask_f = mask.astype(np.float32) / 255.0
            mask_3c = np.stack([mask_f] * 3, axis=-1)
            orig_patch = f[y1:y2, x1:x2].astype(np.float32) / 255.0
            pred_f = p_resized.astype(np.float32) / 255.0
            blended = pred_f * mask_3c + orig_patch * (1 - mask_3c)
            f[y1:y2, x1:x2] = (blended * 255).astype(np.uint8)

            out.write(f)

    out.release()

    # === Sauvegarde des positions dans un JSON ===
    os.makedirs("temp", exist_ok=True)
    with open("temp/circle_positions.json", "w") as fp:
        json.dump(circle_positions, fp, indent=2)
    print(f"Saved circle positions for {len(circle_positions)} frames in temp/circle_positions.json")
